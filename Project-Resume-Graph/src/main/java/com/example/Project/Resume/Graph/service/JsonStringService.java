...
public class JsonStringService {
    ...
    public JSONObject extractShortDurationFromLongDuration(String duration){
    ...
    public JSONObject getLongDuration(String duration){
        ...
        if(matchFound){
            JSONObject jsonObject = new JSONObject();
                    jsonObject.put("starting", extractShortDurationFromLongDuration(matcher.group(1)));
                    if(extractShortDurationFromLongDuration(matcher.group(2))==null)
                        jsonObject.put("ending","present");
                    else
                        jsonObject.put("ending", extractShortDurationFromLongDuration(matcher.group(2)));
